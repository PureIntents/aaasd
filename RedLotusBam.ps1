...
# Obtiene la ruta del archivo en la carpeta Downloads o Descargas del perfil de usuario actual
$filePath = if ($PSUICulture -eq "es-ES") {
    $downloadsPath = Join-Path -Path $env:USERPROFILE -ChildPath "Descargas"
    Get-ChildItem -Path $downloadsPath -Filter "launcher.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
} else {
...
if ($filePath) {
    Write-Host "El archivo existe en la ruta especificada: $filePath"
    # Ejecuta el archivo
    & $filePath
